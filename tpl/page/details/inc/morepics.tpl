        ...
        <ul class="[{if $iMorePics > 4}]slides[{else}]details-picture-more-list[{/if}]">
            ...
            [{foreach from=$oView->getIcons() key="iPicNr" item="oArtIcon" name="sMorePics"}]
                ...
                <li class="details-picture-more-list-item">
                    <a id="morePics_[{$smarty.foreach.sMorePics.iteration}]"  class="details-picture-more-link[{if $smarty.foreach.sMorePics.first}] selected[{/if}]" href="[{$oPictureProduct->getPictureUrl($iPicNr)}]" data-bs-num="[{$smarty.foreach.sMorePics.iteration}]"[{if $aPictureInfo}] data-bs-width="[{$aPictureInfo.0}]" data-bs-height="[{$aPictureInfo.1}]"[{/if}] data-bs-zoom-url="[{$oPictureProduct->getMasterZoomPictureUrl($iPicNr)}]">
